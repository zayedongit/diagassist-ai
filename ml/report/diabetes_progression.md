...
| --- | --- | --- |
| R² | **0.36** | explains 36% of the variation in progression |
| Cross-val R² | **0.40 ± 0.09** | stable across folds → not overfit |
| RMSE | **59.5** | typical error, vs **74.9** for a no-model baseline (~21% lower) |
| MAE | 46.6 | mean absolute error |

A random forest scored no better (R² 0.35), so the transparent linear model wins.
...
| --- | --- | --- |
| BMI | +30.6 | raises progression (strongest) |
| Blood pressure | +16.6 | raises |
| HDL cholesterol | −14.3 | **lowers** (protective) |
| Blood glucose | +7.7 | raises |
| LDL cholesterol | −3.0 | weak (entangled with HDL) |
| Age | +0.4 | negligible here |
...
honest R²≈0.36 model, not an overfit one claiming to be perfect.

## In the app

When a report yields at least two of the six inputs, DiagAssist shows a
