...
const getStatusIcon = (status: string) => {
  switch (status?.toLowerCase()) {
    ...
    default:
      return <Heart className="w-5 h-5 text-foreground" />;
  }
};
    ...
    <Card className="border-warning/30 bg-gradient-to-r from-warning/10 to-warning/5">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-3">
          {statusIcon}
          <div>
            <h2 className="text-lg font-semibold text-foreground">Summary in Simple Terms</h2>
            <p className="text-sm font-normal text-foreground">Easy-to-understand overview of your results</p>
          </div>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="bg-primary/5 rounded-lg p-3 border border-primary/20">
          <p className="text-xs text-foreground">
            <strong className="text-foreground">Remember:</strong> This summary provides a general overview. 
