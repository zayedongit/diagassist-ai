    ...
    <Card className="border-2 shadow-elegant overflow-hidden">
      ...
      <CardHeader className={cn("pb-4", statusInfo.bgColor, statusInfo.borderColor, "border-b-2")}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <CardTitle className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">
                {statusInfo.label}
              </CardTitle>
          ...
          <div className="flex gap-3 sm:gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-600">Panel{abnormalPanels.length !== 1 ? 's' : ''}</div>
            </div>
      ...
      <CardContent className="p-4 sm:p-6 space-y-6">
            ...
            {expandedSections.abnormalities && (
              <div className="space-y-4 animate-fade-in">
                {abnormalPanels.map((panel, panelIndex) => (
                  <div key={panelIndex} className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              {lab.significance && (
                                <p className="text-xs text-gray-600 mt-0.5">{lab.significance}</p>
                              )}
                            </div>
                          </div>
                    ...
                    {panel.interpretation && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        <p className="text-sm text-gray-800">{panel.interpretation}</p>
                      </div>
                    )}
                  </div>
                ))}
        ...
        {clinicalAssessmentData && (
          <div className="space-y-3">
            <button
              ...
              className="w-full flex items-center justify-between p-3 bg-gradient-to-r from-purple-50 to-indigo-50 border-2 border-purple-200 rounded-xl hover:shadow-md transition-all"
            >
              <div className="flex items-center gap-2">
                    ...
                    <div className="space-y-2">
                      {clinicalAssessmentData.investigations.map((investigation: any, index: number) => (
                        <div key={index} className="flex items-center justify-between p-2 bg-blue-50 rounded-lg">
                          <span className="text-sm font-medium text-gray-900">{investigation.test}</span>
                    ...
                    <div className="space-y-2">
                      {clinicalAssessmentData.referrals.map((referral: any, index: number) => (
                        <div key={index} className="flex items-center justify-between p-2 bg-indigo-50 rounded-lg">
                          <span className="text-sm font-medium text-gray-900">{referral.specialty}</span>
                          <Badge variant="outline" className="text-xs">
                            {referral.timeframe}
                          </Badge>
                        </div>
                  ...
                  <div className="bg-white border border-yellow-200 rounded-xl p-4">
                    ...
                    <ul className="space-y-2">
                      {clinicalAssessmentData.redFlags.slice(0, 4).map((flag: string, index: number) => (
                        <li key={index} className="text-sm text-gray-800 flex items-start gap-2 p-2 bg-yellow-50 rounded">
        ...
        <div className="space-y-3">
          ...
          {expandedSections.interpretation && (
            <div className="bg-white border border-green-200 rounded-xl p-4 space-y-4 animate-fade-in">
              <div className="space-y-3">
              ...
              {analysisData.patientFriendlySummary && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h5 className="font-semibold text-gray-900 mb-2">In Simple Terms:</h5>
                  <p className="text-sm text-gray-800 leading-relaxed">
                    {analysisData.patientFriendlySummary}
                  </p>
                </div>
        ...
        {clinicalAssessmentData && clinicalAssessmentData.management && (
          <div className="space-y-3">
            <button
              ...
              className="w-full flex items-center justify-between p-3 bg-gradient-to-r from-cyan-50 to-blue-50 border-2 border-cyan-200 rounded-xl hover:shadow-md transition-all"
            >
              <div className="flex items-center gap-2">
                  ...
                  <div className="bg-white border border-green-200 rounded-xl p-4">
                    ...
                    <ul className="space-y-2">
                      {clinicalAssessmentData.management.diet.map((item: string, index: number) => (
                        <li key={index} className="text-sm text-gray-800 flex items-start gap-2 p-2 bg-green-50 rounded">
                  ...
                  <div className="bg-white border border-blue-200 rounded-xl p-4">
                    ...
                    <ul className="space-y-2">
                      {clinicalAssessmentData.management.lifestyle.map((item: string, index: number) => (
                        <li key={index} className="text-sm text-gray-800 flex items-start gap-2 p-2 bg-blue-50 rounded">
                  ...
                  <div className="bg-white border border-purple-200 rounded-xl p-4">
                    ...
                    <ul className="space-y-2">
                      {clinicalAssessmentData.management.generalRx.map((item: string, index: number) => (
                        <li key={index} className="text-sm text-gray-800 flex items-start gap-2 p-2 bg-purple-50 rounded">
