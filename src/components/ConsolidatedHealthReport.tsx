        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-foreground">{statusInfo.description}</p>
            </div>
          </div>
          ...
          <div className="flex gap-3 sm:gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-foreground">Panel{abnormalPanels.length !== 1 ? 's' : ''}</div>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-foreground">Abnormal</div>
            </div>
          </div>
        </div>
      </CardHeader>
            ...
            {expandedSections.abnormalities && (
              <div className="space-y-4 animate-fade-in">
                {abnormalPanels.map((panel, panelIndex) => (
                  <div key={panelIndex} className="bg-card border border-white/10 rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      {panel.abnormalLabs.map((lab, labIndex) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="font-medium text-foreground text-sm">{lab.name}</p>
                              {lab.significance && (
                                <p className="text-xs text-foreground mt-0.5">{lab.significance}</p>
                              )}
                            </div>
                          </div>
                    ...
                    {panel.interpretation && (
                      <div className="bg-card border border-white/10 rounded-lg p-3">
                        <p className="text-sm text-foreground">{panel.interpretation}</p>
                      </div>
                    )}
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
                  ...
                  <div className="bg-card border border-white/10 rounded-xl p-4">
                    ...
                    <div className="space-y-2">
                      {clinicalAssessmentData.investigations.map((investigation: any, index: number) => (
                        <div key={index} className="flex items-center justify-between p-2 bg-card rounded-lg">
                          <span className="text-sm font-medium text-foreground">{investigation.test}</span>
                ...
                {clinicalAssessmentData.referrals && 
                 clinicalAssessmentData.referrals.length > 0 && (
                  <div className="bg-card border border-indigo-200 rounded-xl p-4">
                    <h4 className="font-semibold text-foreground mb-3 flex items-center gap-2">
                    ...
                    <div className="space-y-2">
                      {clinicalAssessmentData.referrals.map((referral: any, index: number) => (
                        <div key={index} className="flex items-center justify-between p-2 bg-card rounded-lg">
                          <span className="text-sm font-medium text-foreground">{referral.specialty}</span>
                          <Badge variant="outline" className="text-xs">
                            {referral.timeframe}
                          </Badge>
                        </div>
                  ...
                  <div className="bg-card border border-yellow-200 rounded-xl p-4">
                    ...
                    <ul className="space-y-2">
                      {clinicalAssessmentData.redFlags.slice(0, 4).map((flag: string, index: number) => (
                        <li key={index} className="text-sm text-foreground flex items-start gap-2 p-2 bg-yellow-50 rounded">
                  ...
                  <div className="bg-card border border-green-200 rounded-xl p-4">
                    ...
                    <ul className="space-y-2">
                      {clinicalAssessmentData.management.diet.map((item: string, index: number) => (
                        <li key={index} className="text-sm text-foreground flex items-start gap-2 p-2 bg-green-50 rounded">
                  ...
                  <div className="bg-card border border-white/10 rounded-xl p-4">
                    ...
                    <ul className="space-y-2">
                      {clinicalAssessmentData.management.lifestyle.map((item: string, index: number) => (
                        <li key={index} className="text-sm text-foreground flex items-start gap-2 p-2 bg-card rounded">
