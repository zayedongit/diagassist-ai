...
export const SystemScoreBreakdown = ({ breakdown }: SystemScoreBreakdownProps) => {
  ...
  return (
    <div className="space-y-4">
      <h4 className="font-semibold text-foreground text-lg">Detailed System Analysis</h4>
        ...
        {Object.entries(systemScores).map(([systemName, systemData]: [string, SystemScore]) => {
          ...
          return (
            <AccordionItem 
              key={systemName} 
              value={systemName}
              className="border border-white/10 rounded-lg px-4 bg-card"
            >
              <AccordionTrigger className="hover:no-underline">
                <div className="flex items-center gap-3 w-full">
                  <Icon className="w-5 h-5 text-primary" />
                  <div className="flex-1 text-left">
                    <div className="font-semibold text-foreground">{getSystemLabel(systemName)}</div>
              ...
              <AccordionContent className="pt-4 space-y-4">
                ...
                <div>
                  <h5 className="text-sm font-semibold text-foreground mb-2">Parameters Evaluated</h5>
      ...
      <div className="bg-card/[0.03] border border-white/10 rounded-lg p-4 mt-4">
        <h5 className="text-sm font-semibold text-foreground mb-2">Medical Guidelines Referenced</h5>
