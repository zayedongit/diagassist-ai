  ...
  const contexts: Record<string, ParameterContext> = {
    ...
    'potassium': {
      whatItMeans: `Potassium of ${lab.value} ${lab.unit || 'mmol/L'} is crucial for nerve and muscle function. ${
        value > 6.0 ? 'This is severely elevated (hyperkalemia) - MEDICAL EMERGENCY. Can cause fatal cardiac arrhythmias. Immediate treatment required.' :
        value > 5.5 ? 'This is moderately elevated. Risk of cardiac arrhythmias. Requires urgent evaluation and treatment.' :
        value > 5.0 ? 'This is mildly elevated. Monitor closely and identify the cause.' :
        value < 2.5 ? 'This is severely low (severe hypokalemia) - URGENT. Risk of life-threatening arrhythmias and muscle paralysis.' :
        value < 3.0 ? 'This is moderately low. Can cause muscle weakness, cramping, and cardiac issues.' :
        value < 3.5 ? 'This is mildly low. May cause fatigue, muscle weakness, and constipation.' :
        'This is normal (3.5-5.0 mmol/L).'
      } Potassium regulates heart rhythm, muscle contractions, and nerve signals.`,
      possibleCauses: value > 5.0 ? ['Chronic kidney disease (most common cause - kidneys cannot excrete potassium)', 'Acute kidney injury', 'Medications (ACE inhibitors, ARBs, potassium-sparing diuretics, NSAIDs)', 'Addison disease (adrenal insufficiency)', 'Type 1 diabetes (diabetic ketoacidosis)', 'Rhabdomyolysis (muscle breakdown releasing potassium)', 'Hemolysis (RBC destruction)', 'Excessive potassium intake/supplementation', 'Metabolic acidosis'] : ['Diuretics (thiazides, loop diuretics)', 'Vomiting or diarrhea (GI losses)', 'Primary aldosteronism (Conn syndrome)', 'Cushing syndrome', 'Low dietary potassium intake', 'Diabetic ketoacidosis (insulin deficiency)', 'Renal tubular acidosis', 'Hypomagnesemia', 'Excessive licorice consumption'],
      bodyConnection: `At ${lab.value} ${lab.unit || 'mmol/L'}, ${value > 6.0 ? 'severely elevated potassium depolarizes cardiac cells, disrupting electrical conduction. This causes life-threatening arrhythmias including ventricular fibrillation and asystole (cardiac arrest). ECG changes progress from peaked T-waves → wide QRS → sine wave → cardiac arrest. Also causes muscle weakness and paralysis. Requires immediate treatment: insulin+glucose, calcium gluconate, dialysis if renal failure.' : value > 5.0 ? 'elevated potassium affects cardiac conduction, increasing risk of dangerous arrhythmias. May cause muscle weakness, nausea, palpitations. ECG shows peaked T-waves. Requires prompt treatment to prevent progression to severe hyperkalemia: stop offending medications, potassium binders, treat underlying cause.' : value < 2.5 ? 'severely low potassium causes muscle weakness, paralysis, ileus (intestinal paralysis), respiratory failure, and life-threatening cardiac arrhythmias (ventricular tachycardia, torsades de pointes). ECG shows U-waves, flattened T-waves, ST depression. Urgent IV potassium replacement needed.' : value < 3.5 ? 'low potassium impairs muscle contraction causing fatigue, weakness, leg cramps, constipation. Cardiac effects include palpitations, arrhythmias, and increased digoxin toxicity risk. Chronic hypokalemia causes kidney damage and increased blood pressure. Oral potassium supplementation corrects levels.' : 'normal potassium maintains proper nerve conduction, muscle contraction (including heart), and cellular function. Tightly regulated by kidneys via aldosterone.'}`
    },
    'sodium': {
      whatItMeans: `Sodium of ${lab.value} ${lab.unit || 'mmol/L'} regulates fluid balance. ${
        value > 160 ? 'This is severely elevated (severe hypernatremia) - EMERGENCY. Risk of brain damage. Immediate treatment required.' :
        value > 150 ? 'This is significantly elevated. Can cause neurological symptoms. Urgent evaluation needed.' :
        value > 145 ? 'This is mildly elevated. Requires investigation of cause.' :
        value < 120 ? 'This is severely low (severe hyponatremia) - EMERGENCY. Risk of seizures, coma, brain herniation.' :
        value < 125 ? 'This is significantly low. Can cause confusion, seizures. Urgent treatment required.' :
      ...
      possibleCauses: value > 145 ? ['Dehydration (inadequate water intake, fever, excessive sweating)', 'Diabetes insipidus (ADH deficiency or resistance)', 'Excessive salt intake', 'Cushing syndrome (excess cortisol)', 'Primary aldosteronism', 'Osmotic diuresis (uncontrolled diabetes)', 'Loss of free water (diarrhea without adequate replacement)', 'Hyperventilation'] : ['SIADH (syndrome of inappropriate ADH secretion)', 'Diuretics (especially thiazides)', 'Heart failure (fluid retention)', 'Cirrhosis (liver disease with ascites)', 'Kidney disease', 'Adrenal insufficiency (Addison disease)', 'Hypothyroidism', 'Excessive water intake (psychogenic polydipsia)', 'Medications (SSRIs, carbamazepine)', 'Post-operative state'],
      bodyConnection: `At ${lab.value} ${lab.unit || 'mmol/L'}, ${value > 160 ? 'severely elevated sodium causes brain cells to shrink as water moves out, causing altered mental status, seizures, coma, and potentially brain damage or death. Symptoms: lethargy, confusion, weakness, seizures. Correction must be gradual to prevent cerebral edema. Treatment: IV hypotonic fluids, treat underlying cause (give ADH for diabetes insipidus).' : value > 145 ? 'elevated sodium indicates hypertonicity, causing water to shift from cells. Brain cells shrink causing neurological symptoms: thirst, confusion, weakness, muscle twitching. Chronic hypernatremia less symptomatic. Treatment: increase free water intake, IV hypotonic fluids, treat underlying cause.' : value < 120 ? 'severely low sodium causes brain swelling (cerebral edema) as water moves into cells. This causes headache, confusion, seizures, coma, respiratory arrest, brain herniation, and death. Requires immediate treatment in ICU with hypertonic saline. Too rapid correction causes osmotic demyelination syndrome (permanent brain damage).' : value < 135 ? 'low sodium causes cells to swell as water moves in. Symptoms depend on severity and rate: mild (fatigue, nausea, muscle cramps), moderate (confusion, weakness), severe (seizures, coma). Chronic hyponatremia is better tolerated. Treatment: fluid restriction (if SIADH), treat underlying cause, sometimes IV saline. Gradual correction prevents complications.' : 'normal sodium maintains osmotic balance, regulates blood pressure and volume, enables nerve impulses and muscle contractions. Tightly regulated by ADH, aldosterone, and kidneys.'}`
    }
  };
